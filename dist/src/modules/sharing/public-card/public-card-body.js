...
exports.buildCardInitials = buildCardInitials;
exports.renderPublicCardBody = renderPublicCardBody;
exports.buildInlineScript = buildInlineScript;
...
function buildCardInitials(params) {
...
function renderAvatar(params, className, accentColor) {
...
function resolveButtonSkins(layout) {
    switch (layout) {
        case 'bubble':
        case 'arc':
...
function resolveSocialSkin(layout) {
    switch (layout) {
        case 'bubble':
        case 'arc':
            return 'social-soft';
...
function renderActionIcon(icon) {
    const svgOpen = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">';
    const paths = {
...
function renderSocials(params) {
    ...
    const links = params.socialLinks
        .map((link) => {
        const label = escapeHtml(socialLabel(link.platform, link.label ?? undefined));
...
function socialLabel(platform, label) {
    if (label?.trim()) {
...
function buildVCardDataUri(params) {
    const lines = [
        'BEGIN:VCARD',
        'VERSION:3.0',
        `FN:${params.fullName}`,
    ...
    if (params.phone?.trim())
        lines.push(`TEL:${params.phone.trim()}`);
    if (params.subtitle.trim())
        lines.push(`TITLE:${params.subtitle.trim()}`);
    lines.push(`URL:${params.pageUrl}`);
    lines.push('END:VCARD');
    return `data:text/vcard;charset=utf-8,${encodeURIComponent(lines.join('\n'))}`;
}
function renderIdentity(params) {
...
function renderStandardBody(params) {
    const avatarClass = params.design.layout === 'blancLight'
        ? 'avatar-accent-fill'
...
function renderBubbleBody(params) {
...
function renderArcAvatar(params) {
    const safeInitials = escapeHtml(params.initials);
    const accent = escapeAttr(params.design.accentColor);
...
function renderPublicCardBody(params) {
...
function buildInlineScript(pageUrl) {
...
//# sourceMappingURL=public-card-body.js.map